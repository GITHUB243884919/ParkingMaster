...
namespace Game
{
    public class EntityManagerTick : TickBase
    {
        public override void Tick(int deltaTimeMS)
        {
			...
			//UnityEngine.Debug.LogErrorFormat("========begin tick {0}==========", GameManager.GetInstance().tickCount);
			foreach (var val in this.entityManager.entityMovables.Values)
            {
				//UnityEngine.Debug.LogErrorFormat("{0}, {1}", val.entityID, val.mainGameObject.GetInstanceID());
				val.Tick(deltaTimeMS);
            }
			//UnityEngine.Debug.LogErrorFormat("--------end tick{0}---------", GameManager.GetInstance().tickCount);
    ...
    public class EntityManager : Singleton<EntityManager>, ISingleton
    {
        ...
        public void Release()
        {
            ...
            EntityDrivedCar.pool.RecoveryAll();
        }
        ...
        public void Release(EntityMovable entity)
        {
            ...
            {
                case EntityFuncType.DrivedCar:
                    EntityDrivedCar.pool.Delete(entity as EntityDrivedCar);
                    break;
                default:
        ...
        public EntityGameObject GenEntityGameObject(int entityResType, EntityFuncType entityFuncType)
        {
            ...
            {
                case EntityFuncType.DrivedCar:
                    entity = EntityDrivedCar.pool.New();
                    break;
                default:
        ...
        public void AfterTickMoveables()
        {
            ...
            foreach (var v in this.entityMovableRemoves)
            {
                ...
                {
                    case EntityFuncType.DrivedCar:
                        EntityDrivedCar.pool.Delete(v as EntityDrivedCar);
                        break;
                    default:
                        string e = string.Format("没有这种 EntityFuncType {0}", eEntityFuncType);
                        throw new System.Exception(e);
